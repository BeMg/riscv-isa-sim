...
switch (mask) {
    case 0b00:
        for(int i=0; i < vl; i++) {
            float tmp = (*(float *)&rs1.data[0] < *(float *)&rs2.data[0]) ? *(float *)&rs2.data[0] : *(float *)&rs1.data[0];
            float tmp2 = enable[i] ? tmp : 0;
            rst.data[i] = *(int*)&tmp2;
        }
        break;
    case 0b01:
    ...
    case 0b11:
        for(int i=0; i < vl; i++) {
            float tmp = (*(float *)&rs1.data[i] < *(float *)&rs2.data[i]) ? *(float *)&rs2.data[i] : *(float *)&rs1.data[i];
