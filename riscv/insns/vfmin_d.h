...
switch (mask) {
    case 0b00:
        for(int i=0; i < vl; i++) {
            double tmp = (*(double *)&rs1.data[0] > *(double *)&rs2.data[0]) ? *(double *)&rs2.data[0] : *(double *)&rs1.data[0];
    ...
    case 0b11:
        for(int i=0; i < vl; i++) {
            double tmp = (*(double *)&rs1.data[i] > *(double *)&rs2.data[i]) ? *(double *)&rs2.data[i] : *(double *)&rs1.data[i];
            double tmp2 = enable[i] ? tmp : 0;
